...
parser = argparse.ArgumentParser(description = """bridge_netcdf2png.py
...
parser.add_argument('--variableBRIDGE', dest = 'variableBRIDGE', help = 'Variable name in BRIDGE files')
parser.add_argument('--variableOUT', dest = 'variableOUT', help = 'Variable name following CMIP convention')
parser.add_argument('--experiment', dest = 'experiment', help = 'Name of BRIDGE simulation')
parser.add_argument('--reference_file', dest = 'reference_file', help = 'Name of reference simulation for anomalies')
...
if 'ht' in inputDims: inputDims.remove('ht')
if 'surface' in inputDims: inputDims.remove('surface')
if 'unspecified' in inputDims: inputDims.remove('unspecified')
if 'pseudo_1' in inputDims: inputDims.remove('pseudo_1')

if (args.variableOUT == 'orog'):
  if 't' in inputDims: inputDims.remove('t')
...
if (args.variableOUT == 'tas'):
  ...
  outputData_ym[:,:]   = norm(inputData_ym, 0., 1.) * 255 
  outputData_mm[:,:]   = norm(inputData_mm, 0., 1.) * 255
elif (args.variableOUT == 'tos'):
  outputData_ym[:,:]   = norm(inputData_ym, -2., 42.) * 255 
  outputData_mm[:,:]   = norm(inputData_mm, -2., 42.) * 255
elif (args.variableOUT == 'mlotst'):
  outputData_ym[:,:]   = norm(inputData_ym, 0., 1000.) * 255 
  outputData_mm[:,:]   = norm(inputData_mm, -2., 1000.) * 255
elif (args.variableOUT == 'siconc'):
  outputData_ym[:,:]   = norm(inputData_ym, 0., 1.) * 255 
  outputData_mm[:,:]   = norm(inputData_mm, 0., 1.) * 255
  ...
  outputData_ym[:,:]   = norm(inputData_ym, -5000., 5000.) * 255
#   outputData_ym[:,:]   = norm(inputData_ym, -10000., 10000.) * 65535
elif (args.variableOUT == 'bathy'):
  outputData_ym[:,:]   = norm(inputData_ym, -5000., 0.) * 255 
elif (args.variableOUT == 'orog'):
  outputData_ym[:,:]   = norm(inputData_ym, 0., 5000.) * 255 
...
if (inputRank > 2):
  ...
  for index in range(12):
    outputData_mm[:,index*len(inputLon) : ((index+1)*len(inputLon))] = norm(inputData[index,:,:], lowerBound, upperBound) * 255
...
img_ym.save(args.outputDir + args.experiment + '_' + args.variableOUT + '.ym.png')
...
#   outputData_mm = np.where(indMissingValues_mm, 255, outputData_mm)
  ...
  img_mm.save(args.outputDir + args.experiment + '_' + args.variableOUT + '.mm.png')